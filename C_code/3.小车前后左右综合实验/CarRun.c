...
void run(int time)
{
  ...
  softPwmWrite(Left_motor_pwm, 200);   //PWM比例0-255调速，左右轮差异略增减

  //右电机前进
  digitalWrite(Right_motor_go, HIGH);  //右电机前进使能
  digitalWrite(Right_motor_back, LOW); //右电机后退禁止
  softPwmWrite(Right_motor_pwm, 200);  //PWM比例0-255调速，左右轮差异略增减
...
void left(int time)
{
  ...
  digitalWrite(Right_motor_go, HIGH);  //右电机前进使能
  digitalWrite(Right_motor_back, LOW); //右电机后退禁止
  softPwmWrite(Right_motor_pwm, 200);  //右边电机速度设200(0-255)
...
void right(int time)
{
  ...
  digitalWrite(Left_motor_go, HIGH);    //左电机前进使能
  digitalWrite(Left_motor_back, LOW);   //左电机后退禁止
  softPwmWrite(Left_motor_pwm, 200);    //左边电机速度设200(0-255)
