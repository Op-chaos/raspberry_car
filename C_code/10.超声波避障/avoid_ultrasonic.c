...
void main()
{
  ...
  while(1)
  {
   ...
   if (distance > 55)
   {
     run(255, 255);      //当距离障碍物较远时全速前进
   }
   else if (distance >= 30 && distance <= 55)
   {
     run(200, 200);      //当快靠近障碍物时慢速前进
   }
   else if (distance < 30)
   {
     ...
     if (distance >= 30)
     {
       run(200, 200);    //转弯后当前方距离大于25cm时前进
     }
     else if (distance < 30)
     { 
       spin_left(800);     //转弯后前方距离小于25cm时向左原地转弯180度
       brake(1);
      distance =  Distance_test();  //再次测试判断前方距离
       if (distance >= 30)
       {
       ...
       {
         spin_left(400); //转弯后前方距离小于25cm时向左原地转弯90度
         brake(1);
       }
     }
   }
